...
import { EventTileE2eViewModel } from "./EventTileE2eViewModel";

function makeClient(
    getEncryptionInfoForEvent = vi.fn<(event: MatrixEvent) => Promise<EventEncryptionInfo | null>>(),
...
describe("EventTileE2eViewModel", () => {
    ...
    it("uses the replacing event for verification after edits", async () => {
        ...
        vi.spyOn(mxEvent, "isEncrypted").mockReturnValue(true);
        vi.spyOn(replacementEvent, "isEncrypted").mockReturnValue(true);
        const getEncryptionInfoForEvent = vi.fn(async (event: MatrixEvent) => {
            return {
    ...
    it("does not re-verify when a different user's verification changes", async () => {
        const mxEvent = makeEvent();
        vi.spyOn(mxEvent, "isEncrypted").mockReturnValue(true);
        const getEncryptionInfoForEvent = vi.fn().mockResolvedValue({
            shieldColour: EventShieldColour.GREY,
            shieldReason: EventShieldReason.UNSIGNED_DEVICE,
        } as EventEncryptionInfo);
        const cli = makeClient(getEncryptionInfoForEvent);
    ...
    it("ignores stale async verification results", async () => {
        ...
        const getEncryptionInfoForEvent = vi
            .fn()
